...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  /**
   * This private method is called when a new red node is inserted into the tree to ensure that the
   * RB-tree requirements are still being followed.
   * 
   * @param redNode a newly inserted red Node in the tree
   */
  private void enforceRBTreePropertiesAfterInsert(Node<T> redNode) {
    // if root node is red, it will immediately be switched to black when this method is returned
    if (redNode == null || redNode.equals(root))
      return;

    // initialize variables for grandparent, uncle, and parent for easier-to-read code
    boolean isLeftChild = redNode.isLeftChild();
    ...
    Node<T> uncle = null;
    boolean uncleIsLeftChild = false;
    if (grandparent != null) {
    ...
    // case 1: parent's sibling is black or null & is on the opposite side of the violating node
    if (parent != null && grandparent != null) {
      if (!parent.isBlack && isLeftChild != uncleIsLeftChild && (uncle == null || uncle.isBlack)) {
        // rotate and color swap parent and grandparent
        rotate(parent, grandparent);
        parent.isBlack = true;
        grandparent.isBlack = false;
      }
      // case 2: parent's sibling is black or null & is on the same side of the violating node
      else if (!parent.isBlack && isLeftChild == uncleIsLeftChild
          && (uncle == null || uncle.isBlack)) {
        // rotate ourselves into a case 1, by rotating the parent/child violation
      ...
      else if (!parent.isBlack && uncle != null && !uncle.isBlack) {
        // make parent and uncle both black, and the grandparent red
        parent.isBlack = true;
        uncle.isBlack = true;
        grandparent.isBlack = false;
        // now check if the grandparent is violating any RB-tree rules
  ...
  private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    // nodes not formatted correctly
    if (parent == null || child == null) {
      Node<T> parentLChild = parent.leftChild;
      Node<T> parentRChild = parent.rightChild;
      if (parentLChild != null && parentRChild != null) {
        if (!parentLChild.equals(child) && !parentRChild.equals(child))
          throw new IllegalArgumentException("Child / parent nodes are not related correctly");
      }
    }
    // right rotation, updating the parent/child structure of the parent, child, and the child's
    // right node
    if (child.isLeftChild()) {
      ...
      // changing the grandparent's child to be updated to the new child
      if (grandparent != null) {
      ...
      if (root == parent)
        root = temp;
    } // left rotation, updating the parent/child structure of the parent, child, and the child's
    // left node
    else {
